...
local Window = Library:CreateWindow{
    Title = `Dragon Hub | Status: Kostenlos | Version [v.beta]`,
    SubTitle = "von ttvkaiser & FLX_Liam",
    TabWidth = 160,
...
Tabs.Home:CreateParagraph("Aligned Paragraph", {
    Title = "---Lokale Player-Konfiguration---",
    Content = "",
...
local Input = Tabs.Home:AddInput("Input", {
    Title = "Geschwindigkeitseingabe",
    Default = tostring(speed),
    Placeholder = "Geben Sie die Geschwindigkeit ein",
    Numeric = true,
    Finished = false,
...
local Toggle = Tabs.Home:AddToggle("MyToggle", {
    Title = "Geschwindigkeit aktivieren",
    Default = false
})
...
local ToggleInfiniteJump = Tabs.Home:AddToggle("Toggle_InfiniteJump", {Title = "Unendlicher Sprung", Default = false})
...
-- No Clip Toggle
local ToggleNoClip = Tabs.Home:AddToggle("Toggle_NoClip", {Title = "Kein Clip", Default = false})
...
local ToggleAutoLift = Tabs.Main:AddToggle("MyToggle_AutoLift", {Title = "Automatischer Lift", Default = false})
...
local LanguageInfo = Tabs.Status:CreateParagraph("LanguageInfo", {
    Title = "Sprache",
    Content = [[
Aktuelle Skriptsprache: Deutsch

Das Skript ist vollständig auf Deutsch geschrieben, um die Benutzerfreundlichkeit und Übersichtlichkeit zu gewährleisten. Alle Beschriftungen, Schaltflächen und Beschreibungen sind auf Deutsch, sodass jeder sie sofort verstehen und verwenden kann.

Unser Discord-Server unterstützt auch Englisch und Spanisch! Falls Deutsch nicht Ihre Muttersprache ist, können Sie gerne vorbeischauen und Hilfe, Updates und Support in Ihrer Muttersprache erhalten.
]]
})

local Paragraph_PlayTime = Tabs.Status:CreateParagraph("Paragraph_PlayTime", {
    Title = "Zeit im Server",
    Content = "Berechnen..."
})
...
task.spawn(function()
    while true do
        ...
        Paragraph_PlayTime:SetContent(string.format("Sie befinden sich seit:\n%02d Minuten und %02d Sekunden auf diesem Server", minutes, seconds))
        task.wait(1)
    end
end)
...
local Credits = Tabs.Credits:CreateParagraph("Credits", {
    Title = "Credits",
    Content = [[
Dieses Skript wurde von ttvkaiser und FLX_liam erstellt.

Wir haben mit der Arbeit begonnen, weil wir das Gefühl hatten, dass die meisten Skripte entweder fehlerhaft, voller Müll oder einfach unvollständig waren. Wir wollten etwas Einfaches, Nützliches und Sauberes schaffen – etwas, das die Leute wirklich nutzen können, ohne sich durch ein Chaos wühlen zu müssen.

Von der Benutzeroberfläche bis zu den Funktionen wurde alles mit großer Sorgfalt entwickelt. Wir haben alles sorgfältig getestet, um sicherzustellen, dass es im Spiel auch wirklich funktioniert. Egal, ob du Statistiken aufbaust oder einfach nur herumprobierst – wir hoffen, dieses Skript verbessert dein Spielerlebnis.

Vielen Dank an alle, die uns unterstützen. Weitere Updates folgen – das ist erst der Anfang.
]]
})
-- Addons:
